"""
This file converts output head from dust3r checkpoints into uniception format.

Special Notice: dust3r have changed their released weights before/after CVPR, and 
uniception uses the checkpoint BEFORE CVPR. So please make sure you are not converting
...
def convert_dust3r_dpt_checkpoints(checkpoint_path, output_folder, output_filename):

    source_ckpt = torch.load(checkpoint_path, map_location='cpu')
    ...
    for head in ["head1", "head2"]:
        ...
        dpt_reg_processor = DPTRegressionProcessor(
        ...
        dpt_reg_processor_path = os.path.join(output_folder, "dpt_reg_processor", output_filename + f"_reg_processor{head[-1]}.pth")
...
def convert_dust3r_linear_checkpoints(checkpoint_path, output_folder, output_filename):
    ...
    for head in ["head1", "head2"]:
        ...
        linear_feature_path = os.path.join(output_folder, "linear_feature_head", output_filename + f"feature_{head}.pth")
...
def convert_mast3r_dpt_checkpoints(checkpoint_path, output_folder, output_filename):
    ...
    for head in ["head1", "head2"]:
        ...
        dpt_feature_weights = {k : v for k, v in dpt_head.items() if not (k.startswith('head') or k.startswith('head_local_features'))}
        ...
        dpt_processor_weights = {k.replace('head.', '') : v for k, v in dpt_head.items() if (k.startswith('head') and not k.startswith('head_local_features'))}
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Convert dust3r checkpoints to uniception format')

    parser.add_argument('--dust3r_checkpoint_path', type=str, required=True, help='Path to the dust3r checkpoint')
    ...
    # convert dust3r 224 linear checkpoint
    dust3r_ckpt_filepath = os.path.join(args.dust3r_checkpoint_path, 'DUSt3R_ViTLarge_BaseDecoder_224_linear.pth')
    convert_dust3r_linear_checkpoints(dust3r_ckpt_filepath, output_folder, "DUSt3R_224_linear")

    # convert dust3r 512 linear checkpoint
    dust3r_ckpt_filepath = os.path.join(args.dust3r_checkpoint_path, 'DUSt3R_ViTLarge_BaseDecoder_512_linear.pth')
    convert_dust3r_linear_checkpoints(dust3r_ckpt_filepath, output_folder, "DUSt3R_512_linear")

    # convert dust3r 512 dpt checkpoint
    dust3r_ckpt_filepath = os.path.join(args.dust3r_checkpoint_path, 'DUSt3R_ViTLarge_BaseDecoder_512_dpt.pth')
    convert_dust3r_dpt_checkpoints(dust3r_ckpt_filepath, output_folder, "DUSt3R_512_dpt")

    # convert mast3r 512 dpt checkpoint
    mast3r_ckpt_path = os.path.join(args.dust3r_checkpoint_path, 'MASt3R_ViTLarge_BaseDecoder_512_catmlpdpt_metric.pth')
    convert_mast3r_dpt_checkpoints(mast3r_ckpt_path, output_folder, "MASt3R_512_dpt")